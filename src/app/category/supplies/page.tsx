  ...
  useEffect(() => {
    const fetchItems = async () => {
      const { data, error } = await supabase
        ...
        .eq("item_category", "supplies"); // Filter by category
  ...
  return (
    <section className="flex flex-col items-center justify-center gap-8 py-16">
      <div className="bg-white rounded-xl shadow-lg p-8 max-w-xl w-full text-center">
        <h1 className="text-4xl font-bold text-blue-300 mb-4">Supplies</h1>
        <p className="text-lg text-gray-700 mb-6">
          School and study supplies, and more.
        </p>
      </div>
      <div className="flex flex-wrap justify-start w-full">
        {items.length > 0 ? (
          ...
          <p className="text-gray-500">No items found under Supplies.</p>
        )}
      </div>
    </section>
  );
